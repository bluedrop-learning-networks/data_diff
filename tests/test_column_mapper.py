...
def test_exact_column_matching():
    ...
    assert mapping == {"id": "id", "name": "name", "price": "price"}


def test_similar_column_matching():
    source1_cols = ["id", "product_name"]
    source2_cols = ["product_id", "name"]
...
def test_validate_mapping():
    ...
    valid_mapping = {"id": "product_id", "name": "product_name", "price": "cost"}
    mapper.validate_mapping(valid_mapping)  # Should not raise

    invalid_mapping = {"id": "invalid_col"}
